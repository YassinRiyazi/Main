...
class initiation():
    ...
    def initiate_torch(self,):
        ...
        self.sup_res_model.load_state_dict(torch.load(os.path.join(script_dir,'Weights','SuperRes.pt'), weights_only=True))
...
if __name__ == "__main__":
    ...
    supres = initiation(_cuda=torch.cuda.is_available())


    img = cv2.imread('Projects/Viscosity/Markov/Images/000007.jpg', cv2.IMREAD_GRAYSCALE)
    if img is None:
        raise ValueError("Could not open input image.")
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    out_img_y = upscale_image(supres, img, kernel)


    cv2.imwrite('Projects/Viscosity/Markov/output_super_res.png', out_img_y)
    import matplotlib.pyplot as plt
    plt.imshow(out_img_y, cmap='gray')
    plt.axis('off')
    plt.show()