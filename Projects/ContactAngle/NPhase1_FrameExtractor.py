...
from Vidoe2Jpg import get_mp4_files,ffmpeg_frame_extractor

if __name__ == "__main__":
    BaseAddress = os.path.join("/media/d2u25/Dont/Teflon")
    OnlyVideos = get_mp4_files(BaseAddress, max_depth=7)
    print(f"Found {len(OnlyVideos)} MP4 files in {BaseAddress}")

    for video_path in tqdm.tqdm(OnlyVideos):
        ...
        frame_path = frame_path.replace('Teflon', 'frames')
