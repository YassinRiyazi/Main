...
def process_image(filepath: str, rotation_matrix: np.ndarray, cropped_height, output_path: str = None,
    ...
    rotated_image[cropped_height-2:, :] = 0  # Set the top part of the image to black
...
def folderBaseLineNormalizer(experiment):
        ...
        image = cv2.imread(os.path.join(experiment, files[10]))
        (h, w) = image.shape[:2]
        center = (w // 2, h // 2)
...
if __name__ == "__main__":
    ...
    """
    for experiment in tqdm.tqdm(glob.glob("/media/d2u25/Dont/frames/280/S2-SNr2.1_D/*")):
